...
@Slf4j
@Component
//当使用kafka时并且配置非批量、手工提交时生效
@ConditionalOnBean(KafkaConfig.class)
@ConditionalOnExpression("${spring.kafka.consumer.batch:true}==false && ${spring.kafka.consumer.enable-auto-commit:true}==false")
public class SingleAndManualKafkaListener {
	...
	public void listenBatchAndManual(ConsumerRecord<?, ?> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
			Acknowledgment ack) {
		try {
