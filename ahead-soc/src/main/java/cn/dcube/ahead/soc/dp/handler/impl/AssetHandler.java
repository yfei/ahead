...
@Service
@Slf4j
public class AssetHandler implements IDPHandler {
	...
	// 资产在redis中的key
	private String asset_key = "soc_asset";

	// 该handler是否生效
	private boolean enable = true;

	public AssetHandler() {
		asset_key = env.getProperty("soc.dp.event.asset.redis-key");
		enable = env.getProperty("soc.dp.event.asset.enable", Boolean.class);
		if (!enable) {
